...
import 'package:simple_calendar/presentation/five_days_calendar/widgets/multi_day_calendar_event_tile.dart';
...
class MultiDaySingleTimelineWithEvents extends StatelessWidget {
  final DateTime date;
  ...
  const MultiDaySingleTimelineWithEvents({
    required this.date,
  ...
  List<Widget> _getMultiple(BoxConstraints constraints) {
    ...
    for (final events in multipleEvents) {
      for (int i = 0; i < events.length; i++) {
        widgets.add(MultiDayCalendarEventTile(
          rowHeight: rowHeight,
